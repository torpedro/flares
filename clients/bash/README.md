...
payload=$(jq -cn --arg id 'backup' --arg title 'Backup failed' \
  '{id:$id,title:$title,severity:"critical"}')
flare_open_issue "$payload"
flare_get_issue backup
flare_list_issues open 100 0
flare_close_issue backup

payload=$(jq -cn --arg title 'Backup' --arg message 'Finished' \
  '{title:$title,message:$message}')
flare_alert "$payload" backup-run-123
```
...
JSON bodies use the documented HTTP fields, including severity, grouping, reminder, and recovery options. Create bodies with `jq --arg`; never interpolate user values into JSON or shell code. Requests are validated by the server.
...
```bash
if result=$(flare_alert "$payload" backup-run-123); then
  printf '%s\n' "$result"
else
  code=$?
