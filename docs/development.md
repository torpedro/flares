...
uv run --project clients/python pytest -q clients/python/tests tests/contract
```
...
Name the test directories explicitly: run from the repository root, pytest does not
read `clients/python/pyproject.toml`, so it would otherwise walk `target/`, where
`cargo package` leaves copies of these same test files.
